...
const (
	...
	// resolves them but has no constructor for either, so using one as a
	// mapping *value* raises ConstructorError. Resolution still succeeds;
	// callers that care must reject them.
	KindMerge
	KindValue
)
...
// notPlain marks the styles that suppress implicit resolution: an explicit tag
// or any form of quoting.
const notPlain = yaml.TaggedStyle | yaml.DoubleQuotedStyle |
...
//
// Quoted and explicitly tagged scalars keep their string value; only plain
// scalars are resolved implicitly. That is what lets an unquoted
// `reviewDate: 2035-01-15` and a quoted `reviewDate: '2035-01-14'` flow
// through this one path and come out rendering the same way.
//
// It returns an error for a scalar PyYAML resolves but then fails to
// construct, which today means an out-of-range calendar date.
...
func ResolvePlain(text string) (Scalar, error) {
	...
	case reBool.MatchString(text):
		return Scalar{Kind: KindBool, Raw: text, Bool: constructBool(text)}, nil
	case reFloat.MatchString(text):
		return Scalar{Kind: KindFloat, Raw: text, Float: constructFloat(text)}, nil
	case reInt.MatchString(text):
		return Scalar{Kind: KindInt, Raw: text, Int: constructInt(text)}, nil
...
// constructBool mirrors SafeConstructor.bool_values, keyed on value.lower().
func constructBool(text string) bool {
...
// constructInt mirrors construct_yaml_int. The regexp has already validated
// the syntax, so the parses below cannot fail.
func constructInt(text string) *big.Int {
	neg, v := splitSign(text)
	out := new(big.Int)

	switch {
	case v == "0":
		return out // Python returns a bare 0 here, sign and all.
	case strings.HasPrefix(v, "0b"):
		out.SetString(v[2:], 2)
	case strings.HasPrefix(v, "0x"):
		out.SetString(v[2:], 16)
	case v[0] == '0':
		out.SetString(v, 8)
...
// dispatching, so ".Inf" and ".NaN" fall out for free.
func constructFloat(text string) float64 {
	neg, v := splitSign(text)
	v = strings.ToLower(v)
	...
	case v == ".inf":
		f = math.Inf(1)
	case v == ".nan":
		return math.NaN() // Python's nan has no meaningful sign here.
	case strings.Contains(v, ":"):
		base := 1.0
		parts := strings.Split(v, ":")
		for i := len(parts) - 1; i >= 0; i-- {
