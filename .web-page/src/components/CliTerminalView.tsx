    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`text-[11px] px-1.5 py-0.5 rounded border font-semibold ${categoryColor[cmd.category]}`}>
                      {cmd.category}
                    </span>
                  </div>
                  <p className="text-[12px] text-slate-500 line-clamp-2">{cmd.description}</p>
                </button>
              );
            })}
          </div>
        ...
        <div className="lg:col-span-8 space-y-4">
          ...
          <div className="bg-white border border-slate-200 shadow-sm rounded-2xl p-5 space-y-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex gap-1">
                  {selectedCommand.exitCodesPossible.map(code => (
                    <span key={code} className="px-1.5 py-0.5 rounded bg-amber-50 text-amber-800 border border-amber-200 text-[11px] font-mono font-bold">
                      {code}
                    </span>
                  ))}
                </div>
            ...
            {selectedCommand.flags.length > 0 && (
              <div className="bg-slate-50 p-3 rounded-xl border border-slate-200 space-y-2">
                <span className="text-[12px] font-bold text-slate-600 uppercase tracking-wider">Command Flags</span>
                  ...
                  {selectedCommand.flags.map(flag => (
                    <div key={flag.flag} className="flex flex-col gap-1">
                      <label className="text-slate-700 font-mono flex items-center gap-1 font-medium">
                        <span>{flag.flag}</span>
                        {flag.required && <span className="text-rose-600 text-[11px]">*req</span>}
                      </label>
                      <input
                        type="text"
            ...
            <div className="bg-indigo-950 p-3 rounded-xl border border-indigo-900 flex items-center justify-between gap-3 font-mono text-xs text-indigo-300 shadow-inner">
              ...
              <div className="flex items-center gap-2 shrink-0">
                <button
                  ...
                  className="p-1.5 rounded-lg bg-indigo-900 hover:bg-indigo-800 text-slate-300 text-xs flex items-center gap-1 border border-indigo-800"
                  title="Copy command"
                >
          ...
          <div className="bg-[#171340] border border-indigo-900 rounded-2xl overflow-hidden shadow-xl flex flex-col h-[350px]">
            {/* Window titlebar */}
            <div className="bg-indigo-950 px-4 py-2 border-b border-indigo-900 flex items-center justify-between text-xs">
              <div className="flex items-center gap-2">
                ...
                <span className="font-mono text-slate-400 text-[12px] ml-2">bash - company-os cli</span>
              </div>
