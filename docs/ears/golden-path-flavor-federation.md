...
| GPF-R-1.6 | THE SYSTEM SHALL ship one golden-path document whose steps match the CLI's printed guidance chain from environment prerequisites (~~Python, pyyaml, PATH~~ → download the binary, PATH) through `prd complete`. **Amended 2026-07-26** — only the illustrative parenthetical changes; the requirement is unchanged. The Python prerequisites it named no longer exist after the Go port ([Amendment 1](federation-enrichment.md#amendment-1--r-74-partial-retirement-2026-07-26)). |
...
| GPF-R-1.9a | WHEN `company-os add team <id> --repair` is run on a team that already exists, THE SYSTEM SHALL write only the scaffolded files that are absent — from the same definitions `add team` creates from, so a repaired file is byte-identical to what creation would have produced — SHALL NOT modify or overwrite any file that is present, and WHERE nothing is absent SHALL change nothing at all, including generated blocks, so that `--repair` is safe to run speculatively. Added 2026-07-26: GPF-R-1.9 refuses outright once the unit exists, which left a team missing a single standards file with no path back short of hand-copying from another team. |
| GPF-R-1.9b | WHEN a repair completes, THE SYSTEM SHALL report the written and the skipped files individually rather than in aggregate, because the skipped list is the evidence that nothing was overwritten and a count alone does not distinguish "already correct" from "not attempted". |
| GPF-R-1.9c | IF `--repair` names a team that does not exist, THE SYSTEM SHALL refuse and exit 3, naming `add team` as the command that creates one; IF `--repair` is combined with any kind other than `team`, THE SYSTEM SHALL exit 2, so that `add platform <id> --repair` cannot appear to have repaired something. |
